...
# 1. Panggil load_dotenv DULUAN sebelum os.getenv apapun
...
def generate_ai_response(user_message, history=None, file_type=None, file_name=None):
    ...
    try:
        if AI_PROVIDER == "openai" and OPENAI_API_KEY:
            # ✅ Disesuaikan: Meneruskan parameter 'history' ke _generate_openai
        ...
        # TAMPILKAN DETAIL ERROR ASLI DI TERMINAL
        print("\n=================== DETEKSI ERROR AI ===================")
        print(f"Provider yang digunakan : {AI_PROVIDER}")
        print(f"Pesan Error Asli        : {exc}")
        print("========================================================\n")

    # Jika API Gagal/Gagal Auth, akan jatuh ke fallback ini
...
def _generate_openai(user_message, history=None, file_type=None, file_name=None):
    """Integrasi OpenRouter dengan Memori Riwayat Chat."""
...
try:
    ...
    pass  # Di Vercel environment variable dibaca otomatis dari dashboard